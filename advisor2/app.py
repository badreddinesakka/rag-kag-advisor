...
import rapport
from decision import CONTRAINT, CONSEQUENCE, DIAGNOSTIC, REGLE, MESURE
from router_preuve import choix_advisor, config_indexation
...
with st.sidebar:
    ...
    # Champ VIDE par défaut, volontairement. L'Advisor ne connaît pas les
    # modèles servis par cette installation : proposer un nom donnerait à une
    # supposition l'apparence d'une recommandation.
    probe_model = st.text_input(
        "Modèle Ollama pour le sondage",
        value="",
        placeholder="nom du modèle servi par ton Ollama",
        help="Aucun défaut : l'Advisor ne décide pas des modèles, seulement "
             "de la configuration que le corpus justifie. Liste tes modèles "
             "avec « ollama list ».")
...
def render_preuve(choix: list):
    ...
    for statut, titre, aide in (
        ...
        # Cinquième niveau : ni valeur ni choix, une observation. Comptées
        # parmi les paramètres, ces lignes faisaient paraître l'Advisor moins
        # mesuré qu'il ne l'est.
        (DIAGNOSTIC, "🔍 Diagnostic",
         "observation, pas un réglage — ne compte pas dans les chiffres ci-dessus"),
    ):
...
elif not probe_model.strip():
    st.warning(
        "Indiquez un modèle Ollama dans la barre latérale pour lancer le "
        "sondage. Sans lui, la décision retombera sur le comptage d'entités, "
        "beaucoup moins fiable."
    )
    probe = None
else:
    probe = probe_cached(payload, probe_model.strip(), n_chunks, per_doc)
...
# L'écran disparaît dès qu'on ferme l'onglet. Une configuration qu'on ne peut
# ni relire dans trois semaines ni montrer à un collègue n'est pas un livrable.
st.subheader("Emporter cette configuration")
_horodatage = datetime.now().strftime("%Y%m%d_%H%M")
_nom_corpus = ", ".join(f.name for f in files)[:60] if files else ""

_col_pdf, _col_txt = st.columns(2)
if rapport.disponible():
    try:
        _pdf = rapport.rapport_pdf(corpus, choix, arch, decision, probe,
                                   _nom_corpus)
        _col_pdf.download_button(
            "Rapport PDF", data=_pdf,
            file_name=f"advisor_{arch.lower()}_{_horodatage}.pdf",
            mime="application/pdf", use_container_width=True)
    except Exception as _e:
        _col_pdf.error(f"PDF impossible : {_e}")
else:
    _col_pdf.info("PDF indisponible — `pip install reportlab`")

_col_txt.download_button(
    "Rapport texte", data=rapport.rapport_texte(corpus, choix, arch,
                                                decision, probe),
    file_name=f"advisor_{arch.lower()}_{_horodatage}.txt",
    mime="text/plain", use_container_width=True)

st.caption("Le rapport contient une section que l'écran n'affiche pas : "
           "la liste des paramètres que l'Advisor NE décide PAS, et pourquoi.")
