...
Trois stratégies de découpage :
  ...
  - "semantic"   : coupe là où le sujet change (embeddings via Ollama,
                   modèle fourni par l'appelant)
...
# Aucun défaut : le modèle vient de l'environnement ou de l'appelant.
# Le découpage sémantique est le seul à en avoir besoin ; sans modèle,
# ce candidat est simplement écarté au lieu d'être noté avec un modèle
# que personne n'a choisi.
EMBED_MODEL = os.environ.get("CHUNKER_EMBED_MODEL")
...
def embed_texts(texts: list[str], model: str | None = EMBED_MODEL) -> list[list[float]]:
    if not model:
        raise ValueError(
            "Aucun modèle d'embedding fourni. Passe --embed-model, ou "
            "définis CHUNKER_EMBED_MODEL. L'Advisor ne choisit pas de "
            "modèle à ta place : il n'a aucun moyen de savoir ce qui "
            "est installé sur cette machine.")
...
def main() -> None:
    ...
    ap.add_argument("--embed-model", default=EMBED_MODEL,
                    help="obligatoire pour le découpage sémantique ; "
                         "sans lui ce candidat est écarté")
