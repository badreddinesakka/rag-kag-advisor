...
DIAGNOSTIC  = "diagnostic"    # ni valeur ni choix : une observation
...
def diagnostic(nom: str, valeur, raison: str) -> Choix:
    """
    Une OBSERVATION, pas un paramètre de configuration.

    « le découpage sémantique n'a produit aucun morceau » ou « l'aptitude d'un
    morceau à porter une relation n'est pas mesurée » ne sont pas des réglages :
    rien ne les applique, ils expliquent pourquoi une mesure manque.

    Les compter parmi les paramètres faussait le décompte du rapport : sur le
    corpus Ooredoo, « 1 mesuré sur 14 » comptait deux diagnostics dans le
    dénominateur et faisait paraître l'Advisor moins mesuré qu'il ne l'est.
    """
    return Choix(nom, valeur, DIAGNOSTIC, raison)
...
if __name__ == "__main__":
    exemple = [
        contraint("embedding_type", "multilingue",
                  "corpus en deux langues — le nom du modèle est "
                  "choisi par l'utilisateur, pas par l'Advisor"),
