...
def index_corpus(files: list[tuple[str, bytes]], config: dict,
    ...
    model  = config.get("_extraction_model") or config.get("extraction_model")
    if not model:
        raise SystemExit(
            "[ECHEC] Aucun modèle d'extraction. L'Advisor ne nomme pas de "
            "modèle : passe --extraction-model avec un modèle servi par "
            "ton Ollama.")
    ...
    # La valeur vient de la config quand l'Advisor l'a produite, pour que
    # ce qui est appliqué soit exactement ce que le rapport annonce.
    overlap_chars = int(config.get("_chunk_overlap_chars_kag")
                        or max(0, chunk_chars // 10))
    ...
    if strategie and mes_chars and not chunk_size:
        ...
        modele_emb = config.get("_embedding_model")
...
def config_from_advisor(files, mutability="figé", use_probe=True,
    ...
    if use_probe:
        ...
        probe_result = probe_corpus(files, model=probe_model)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--extraction-model", required=True,
                    help="modèle Ollama qui extrait les triplets. OBLIGATOIRE : "
                         "l'Advisor ne nomme aucun modèle, il ne sait pas ce qui "
                         "est servi par ton installation.")
    ap.add_argument("--probe-model",
                    help="modèle du sondage (défaut : celui de l'extraction)")
    ...
    if args.config:
        ...
        if not args.no_probe:
            _, probe_data = config_from_advisor(
                corpus_files, args.mutability, True,
                args.probe_model or args.extraction_model)
    else:
        print("[!] Aucun --config : le routeur est refait ici, sans la mesure. "
              "La boucle reste OUVERTE.")
        cfg, probe_data = config_from_advisor(
            corpus_files, args.mutability, not args.no_probe,
            args.probe_model or args.extraction_model)
    # Le modèle donné en ligne de commande FAIT LOI. La config de l'Advisor
    # n'en contient plus : il décide de la stratégie, pas de l'installation.
    cfg["_extraction_model"] = args.extraction_model
