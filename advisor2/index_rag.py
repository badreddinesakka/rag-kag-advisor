...
def parents_mesures(files: list[tuple[str, bytes]], strategy: str,
                    target_chars: int, embed_model: str | None = None) -> list[dict]:
...
def build_chunks(files: list[tuple[str, bytes]], config: dict,
    ...
    if strategy and mes_chars:
        model = config.get("_embedding_model") or config.get("embedding_model")
...
def create_collection(client, name: str, config: dict, chunk_chars: int,
    ...
    # La dimension n'est plus imposée par l'Advisor : il ne connaît pas le
    # modèle. On la LIT sur le modèle réellement fourni, en vectorisant un
    # mot. Supposer 1024 marchait tant qu'un seul modèle était installé ;
    # avec un autre modèle, Milvus rejetait l'insertion sans expliquer pourquoi.
    dim = config.get("_embedding_dim")
    if not dim:
        modele = config.get("_embedding_model") or config.get("embedding_model")
        dim = len(embed_texts(["dimension"], modele)[0])
...
def index_corpus(files: list[tuple[str, bytes]], config: dict,
    ...
    if not model:
        return {"ok": False,
                "error": "Aucun modèle d'embedding dans la config. "
                         "L'Advisor recommande un TYPE de modèle "
                         "(multilingue ou monolingue), pas un nom : "
                         "passe --embed-model avec un modèle de ce type "
                         "servi par ton Ollama."}
    ...
    for start in range(0, total, EMBED_BATCH):
        ...
        if dim_reelle is None and vectors:
            ...
            attendue = config.get("_embedding_dim") or dim_reelle
            if dim_reelle != attendue:
                return {"ok": False,
                        "error": f"Le modèle « {model} » renvoie des vecteurs de "
                                 f"{dim_reelle} dimensions, la collection en attend "
                                 f"{attendue}. Reconstruis la collection avec --recreate, "
                                 f"ou reprends le modèle qui l'a créée."}
    ...
    report = {
        ...
        "embedding_dim": dim_reelle or config.get("_embedding_dim"),
        "index_type": config.get("_index_type", "FLAT"),
        "metric_type": config.get("_metric_type", "COSINE"),
...
if __name__ == "__main__":
    ...
    ap.add_argument("--embed-model", required=True,
                    help="modèle d'embedding Ollama. OBLIGATOIRE : l'Advisor "
                         "recommande un TYPE (multilingue ou monolingue), pas "
                         "un nom — il ne sait pas ce que sert ton installation.")
    ...
    # Le modèle donné en ligne de commande FAIT LOI : la config de
    # l'Advisor ne contient qu'un type, jamais un nom.
    cfg["_embedding_model"] = args.embed_model
