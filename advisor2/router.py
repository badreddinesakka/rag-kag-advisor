...
# --- TYPE de modèle d'embedding ---------------------------------------------
# L'Advisor ne nomme AUCUN modèle. Un nom de modèle décrit une installation, pas
# un corpus : il serait faux dès que l'outil change de machine. Ce que le corpus
# permet de dire, en revanche, c'est de quel TYPE de modèle il a besoin.
# L'utilisateur choisit ensuite un modèle de ce type parmi ceux dont il dispose.
EMBEDDING_MULTILINGUE  = "multilingue"
EMBEDDING_MONOLINGUE   = "monolingue"
...
def _rag_common(c: dict) -> tuple[dict, list[str]]:
    ...
    # On recommande un TYPE, jamais un nom. Le corpus dit s'il faut un modèle
    # capable de plusieurs langues ; il ne dit rien sur ce qui est installé.
    model = EMBEDDING_MULTILINGUE if multi else EMBEDDING_MONOLINGUE
    if multi:
        reasons.append(
            "Corpus multilingue → il faut un modèle d'embedding multilingue : "
            "un modèle entraîné sur une seule langue place les traductions loin "
            "les unes des autres."
        )
    else:
        reasons.append(
            "Corpus monolingue → un modèle monolingue suffit ; un multilingue "
            "convient aussi, au prix d'un peu de finesse."
        )
    ...
    common = {
        ...
        # la dimension dépend du modèle choisi par l'utilisateur,
        # elle n'est donc pas décidée ici
        "_embedding_dim": None,
...
def _rag_versions(c: dict) -> dict:
    ...
    multi = c.get("is_multilingual", False)
    # Pas de nom ici : l'Advisor dit S'IL faut un reranker, pas lequel
    # installer. Et la valeur doit rester une VALEUR — « un reranker de type
    # cross-encoder » était une phrase, qui partait telle quelle dans la config
    # d'indexation là où un booléen est attendu.
    reranker_model = "recommandé"
    versions = {}
...
def decide(corpus: dict, mutability: str = "figé", probe: dict | None = None) -> dict:
    ...
    if words < HARD_MIN_WORDS_KAG:
        kag_score = 0
        kag_reasons = [
            f"Corpus de {_n(words)} mots. En dessous de {_n(HARD_MIN_WORDS_KAG)} mots, "
            f"construire un graphe coûte plus cher que ça ne rapporte : "
            f"l'extraction des triplets se paie une fois pour un corpus qui "
            f"n'a pas assez de liens à exploiter → RAG, sans même sonder."
        ]
